...
import { UserRole, type CreateUserDto, type UserDto, type WhatsappNumberDto } from '@dljobs/shared';
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 p-8">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-wa-green">Управление</h1>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-gray-300">Новый пользователь</h2>
        <form onSubmit={createUser} className="flex flex-wrap items-end gap-2">
          ...
          <button className="rounded bg-wa-green px-4 py-2 text-sm font-medium text-black">Создать</button>
        </form>
        {error && <p className="text-sm text-red-400">{error}</p>}
      </section>

      <section className="space-y-2">
        <h2 className="text-sm font-semibold text-gray-300">Пользователи</h2>
            ...
            <div key={u.id} className="flex items-center justify-between px-4 py-3">
              <div>
                ...
                <span className="ml-2 text-xs text-gray-400">
                  {u.role === UserRole.Admin ? 'админ' : 'оператор'}
                </span>
              </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setEditing(null)}>
          <div className="w-full max-w-sm space-y-3 rounded-lg bg-wa-panel p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-sm font-semibold text-gray-200">Доступные номера</h3>
            <div className="max-h-64 space-y-1 overflow-y-auto">
              ...
              {numbers.length === 0 && <p className="text-xs text-gray-500">Нет номеров</p>}
            </div>
